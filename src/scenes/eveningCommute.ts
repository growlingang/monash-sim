...
const launchMinigame = async (option: CommuteDefinition, store: GameStore, root: HTMLElement) => {
  ...
  store.setState((prev) => {
    ...
    // Always advance to evening activity
...
const renderOptionCard = (
  ...
  card.className = 'evening-commute__option';
  ...
  card.innerHTML = `
    <header>
      <h3>${option.label}</h3>
      <p class="evening-commute__difficulty">${option.difficulty.toUpperCase()}</p>
    </header>
    <ul class="evening-commute__facts">
    ...
    <p class="evening-commute__flavor">${option.successOutcome.description}</p>
  `;

  const button = document.createElement('button');
  button.type = 'button';
  button.className = 'evening-commute__choose';
  button.textContent = afford ? 'Choose commute' : 'Insufficient funds';
...
export const renderEveningCommute = (root: HTMLElement, store: GameStore) => {
  ...
  header.innerHTML = `
    ...
    <p>Time to head home. Choose your transport method.</p>
  `;

  const statsBar = createStatsBar(store.getState());

  const optionsContainer = document.createElement('section');
  optionsContainer.className = 'evening-commute__options';
  ...
  const renderOptions = () => {
    ...
    // Auto-resolve if no paid options are affordable
